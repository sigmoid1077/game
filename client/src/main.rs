...
fn startup(mut connect_event: EventWriter<net::ConnectEvent>) {
    connect_event.send(net::ConnectEvent::new(SocketAddr::new(
...
fn update(
    ...
    for keyboard_input_event in keyboard_input_events.iter() {
        if keyboard_input_event.state == ButtonState::Pressed {
            send_packet_to_server_event.send(net::SendPacketToServerEvent::new(
