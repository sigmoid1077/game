use crate::net::{Packet, TcpListenerComponent, TcpStreamComponent};
use bevy::{
...
fn unbind_event_system(
    ...
    for _ in unbind_events.iter() {
        for (tcp_stream_entity, tcp_stream_component) in
            tcp_stream_entity_and_tcp_stream_component_query.iter()
        {
...
fn send_packet_to_all_clients_event_system(
    ...
    for send_packet_to_all_clients_event in send_packet_to_all_clients_events.iter() {
        for mut tcp_stream_component in tcp_stream_component_query.iter_mut() {
            tcp_stream_component
                .tcp_stream
                .write_all(&bincode::serialize(&send_packet_to_all_clients_event.packet).unwrap())
                .unwrap();
        }
    }
}
...
fn listen_for_client_packets_system(
    ...
    for (tcp_steam_entity, mut tcp_stream_component) in
        tcp_stream_entity_and_tcp_stream_component_query.iter_mut()
    {
        ...
        match tcp_stream_component.tcp_stream.read(&mut buffer) {
            ...
            Ok(packet_length) => {
                recieved_packet_from_client_event.send(RecievedPacketFromClientEvent::new(
